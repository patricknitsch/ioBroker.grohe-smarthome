...
class GroheDeviceManagement extends DeviceManagement {
	...
	_getSenseTemplate(fullId, deviceId) {
		return {
			...
			mainTileItems: {
				...
				humidity: this._stateItem(
					`${fullId}.humidity`,
					{ en: 'Humidity', de: 'Luftfeuchte' },
					'%',
					TILE_ITEM_FONT_SIZE,
				),
				battery: this._stateItem(
					`${fullId}.battery`,
					{ en: 'Battery', de: 'Batterie' },
					'%',
					TILE_ITEM_FONT_SIZE,
				),
			},
	...
	_getGuardTemplate(fullId, deviceId) {
		return {
			...
			mainTileItems: {
				...
				flowRate: this._stateItem(
					`${fullId}.flowRate`,
					{ en: 'Flow rate', de: 'Durchfluss' },
					'l/h',
					TILE_ITEM_FONT_SIZE,
				),
				pressure: this._stateItem(
					`${fullId}.pressure`,
				...
				valveOpen: this._setStateItem(`${fullId}.controls.valveOpen`, { en: 'Open', de: 'Öffnen' }),
				valveClose: this._setStateItem(`${fullId}.controls.valveClose`, { en: 'Close', de: 'Schließen' }),
			},
			infoTabItems: {
	...
	_getBlueTemplate(fullId, deviceId, type) {
		return {
			...
			mainTileItems: {
				...
				lastMeasurement: this._stateItem(
					`${fullId}.lastMeasurement`,
					{
						en: 'Last measurement',
						de: 'Letzte Messung',
					},
	 ...
	 * @param {number} [fontSize] - Optional font size override (e.g. TILE_ITEM_FONT_SIZE)
	 * @returns {object} State config item
	 */
	_stateItem(oid, label, unit, fontSize) {
		const item = {
			type: 'state',
			oid,
			foreign: true,
			label,
			size: fontSize || FULL_WIDTH_SIZE,
			newLine: true,
		};
		if (unit) {
			item.unit = unit;
