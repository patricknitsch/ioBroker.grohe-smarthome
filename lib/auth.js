 ...
 *   2. Parse ALL form fields (hidden CSRF tokens) + action URL from Keycloak HTML
 *   3. POST all fields + username/password with accumulated cookies
...
class GroheAuth {
	...
	async _doLogin(email, password) {
		...
		this.log.debug(`[auth] collected ${Object.keys(cookies).length} cookie(s) after redirect chain`);

		// ── Step 2 ──────────────────────────────────────────────────────
		// Parse form: action URL + ALL input fields (hidden CSRF tokens)
		// ────────────────────────────────────────────────────────────────
		...
		// Collect ALL form fields (hidden inputs like credentialId, CSRF, etc.)
		const formFields = {};
		...
		formFields['username'] = email;
		formFields['password'] = password;

		const fieldNames = Object.keys(formFields).filter(k => k !== 'password');
		this.log.debug(`[auth] step 2: form action -> ${actionUrl.split('?')[0]}, fields: [${fieldNames.join(', ')}]`);

		// ── Step 3 ──────────────────────────────────────────────────────
		// POST credentials with all accumulated cookies
		// ────────────────────────────────────────────────────────────────
		this.log.debug('[auth] step 3: POST credentials');
		const postData = new URLSearchParams(formFields).toString();
		const cookieHeader = this._cookiesToHeader(cookies);

		let location;
		try {
			const resp = await axios.post(actionUrl, postData, {
				timeout: 30000,
				headers: {
			...
			if (resp.status === 200 && typeof resp.data === 'string') {
				...
				this.log.debug('[auth] POST returned 200 HTML (first 300 chars): ' + resp.data.substring(0, 300));
				throw new Error('Login fehlgeschlagen – Keycloak zeigte Login-Seite erneut (status 200)');
			}

			location = resp.headers?.location || '';
			this.log.debug(`[auth] POST status ${resp.status}, location: ${(location || 'NONE').substring(0, 80)}`);
		} catch (err) {
			if (err.response && err.response.status >= 300 && err.response.status < 400) {
				location = err.response.headers?.location || '';
				this.log.debug(`[auth] caught redirect ${err.response.status} -> ${(location || '').substring(0, 60)}`);
			} else if (err.message && err.message.includes('Login fehlgeschlagen')) {
				throw err;
			} else if (err.response) {
				this.log.error(`[auth] POST returned status ${err.response.status}`);
		...
		// ── Step 4 ──────────────────────────────────────────────────────
		// Handle redirect(s) – look for ondus:// scheme
		// ────────────────────────────────────────────────────────────────
	...
	async _getWithCookies(startUrl) {
		...
		for (let i = 0; i < 20; i++) {
			this.log.debug(`[auth] redirect hop ${i}: GET ${currentUrl.split('?')[0]}`);
	 ...
	 * @param {Object<string,string>} cookies – mutated in place
	 */
