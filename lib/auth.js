...
class GroheAuth {
	...
	async _doLogin(email, password) {
		...
		try {
			...
			if (err.response && err.response.status >= 300 && err.response.status < 400) {
				...
				this.log.debug(`[auth] caught redirect ${err.response.status} -> ${(location || '').substring(0, 200)}`);
			} else if (err.message && (err.message.includes('Login fehlgeschlagen') || err.message.includes('Ungültige'))) {
				throw err;
	 ...
	 * @param {tough.CookieJar} jar
	 * @returns {Promise<{body: string, finalUrl: string}>}
	 */
	async _getWithJar(startUrl, jar) {
		...
		for (let i = 0; i < 20; i++) {
			...
			this.log.debug(`[auth]   hop ${i} -> ${resp.status} final (${typeof resp.data === 'string' ? resp.data.length : 0} bytes)`);
			return {
				body: typeof resp.data === 'string' ? resp.data : '',
				finalUrl: currentUrl,
			};
		}
		throw new Error('Zu viele Redirects beim Laden der Login-Seite');
	}
	...
	async _followRedirects(url, baseUrl, jar) {
		...
		throw new Error('Zu viele Redirects nach Login');
	}
	...
	async _exchangeOndusUrl(ondusUrl) {
		...
		this.log.debug(`[auth] step 5: exchanging code for tokens -> ${httpsUrl.substring(0, 100)}`);
	...
	_checkHtmlErrors(html) {
		...
		if (html.includes('Invalid username or password') || html.includes('Invalid email address or password')) {
			throw new Error('Ungültige Zugangsdaten (E-Mail oder Passwort falsch)');
		}
