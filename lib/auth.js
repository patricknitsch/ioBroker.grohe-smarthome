...
class GroheAuth {
	...
	async login(email, password) {
		...
		const loginPage = await axios.get(AUTH_BASE_URL, {
			maxRedirects: 10,
		...
		const actionUrl = new URL(actionRaw, loginPage.request?.res?.responseUrl || AUTH_BASE_URL).toString();
		...
		let postResp;
		try {
			postResp = await axios.post(actionUrl, new URLSearchParams({ username: email, password }).toString(), {
				headers: {
		...
		this.log.debug(`[auth] POST status=${postResp.status} location=${location.substring(0, 40)}…`);
		...
		if (!location.startsWith('ondus://')) {
			...
			if (location && (postResp.status === 302 || postResp.status === 303)) {
				return this._followRedirects(location, actionUrl);
			}
			throw new Error(
	...
	/* ------------------------------------------------------------------ */
	async _followRedirects(url, baseUrl) {
	...
	_applyTokens(data) {
		...
		// subtract 60 s safety margin (same as groheblue)
