...
class GroheAuth {
	...
	constructor(log, setTimeoutFn) {
		this.log = log;
		this.setTimeout = typeof setTimeoutFn === 'function' ? setTimeoutFn : globalThis.setTimeout.bind(globalThis);
	...
	async login(email, password) {
		...
		for (let attempt = 1; attempt <= 3; attempt++) {
			try {
				...
				if (attempt < 3) {
					await new Promise(resolve => this.setTimeout(resolve, attempt * 2000));
				}
			}
		}
		throw lastError;
	...
	async _doLogin(email, password) {
		...
		if (!actionRaw) {
			this.log.warn(`[auth] no <form action> in HTML: ${html.substring(0, 500)}`);
		...
		try {
			...
			if (err.response && err.response.status >= 300 && err.response.status < 400) {
				...
				this.log.warn(`[auth] POST status ${err.response.status}`);
				throw err;
			} else {
				throw err;
			}
