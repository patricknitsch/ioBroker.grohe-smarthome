	...
	if (adapter.config.notifyUseSynochat) {
		try {
			if (!adapter.config.notifyChannelSynochat) {
				adapter.log.warn(
					'[Notification] Synology Chat channel is not configured. Message could not be sent.',
				);
			} else {
 ...
 *
 * 1. Registers the notification with the ioBroker Notification Manager (admin UI integration).
 * 2. Forwards the message to all enabled direct push providers (Telegram, Pushover, etc.)
 *    if `config.notifyEnabled` is true.
 *
 * @param {object} adapter  - The ioBroker adapter instance.
