...
class GroheClient {
	...
	async _discoverViaFallback() {
		let locations;
		try {
			locations = await this._getLocations();
		} catch (err) {
			if (err?.response?.status === 404) {
				this.log.info('[client] /locations returned 404 – trying discovery via /users endpoint');
				locations = await this._getLocationsViaUser();
			} else {
				throw err;
			}
		}
