...
class GroheClient {
	...
	async dumpApiStructure() {
		...
		if (Array.isArray(locations)) {
			...
			try {
				const locs = await this._getLocationsViaUser();
		...
		for (const locId of locationIds) {
			...
			const roomIds = Array.isArray(rooms) ? rooms.map(r => r.id) : [];
			...
			for (const roomId of roomIds) {
				...
				const appIds = Array.isArray(appliances) ? appliances.map(a => a.appliance_id) : [];
				...
				for (const appId of appIds) {
					...
					await this._dumpEndpoint(`${appBase}/notifications?pageSize=1`);
