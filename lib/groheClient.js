...
class GroheClient {
		...
		if (!this._useFallbackDiscovery) {
			try {
				...
				if (err?.response?.status === 404) {
					this.log.info('[client] /dashboard returned 404 – switching to fallback discovery (/locations → /rooms → /appliances)');
	...
	async _getAppliances(locationId, roomId) {
		const resp = await this.request({ method: 'GET', url: `${BASE}/locations/${locationId}/rooms/${roomId}/appliances` });
		return resp.data || [];
	}
