...
class GroheClient {
	...
	async request(config, retry = true) {
		...
		try {
			...
			if (body) {
				this.log.debug(`[client] HTTP ${status} response body for ${config.url}: ${JSON.stringify(body)}`);
			}
			throw err;
		}
	}
	...
	async getDashboard() {
		if (!this._useFallbackDiscovery) {
			try {
				...
				if (err?.response?.status === 404) {
					this.log.info(
						'[client] /dashboard returned 404 – switching to fallback discovery (/locations → /rooms → /appliances)',
					);
		...
		return this._discoverViaLocations();
	}

	async _discoverViaLocations() {
		const locations = await this._getLocations();
		const result = { locations: locations.slice(0) };
		result.locations.length = 0;

		for (const loc of locations) {
			const rooms = await this._getRooms(loc.id);
			const locEntry = { id: loc.id, rooms: rooms.slice(0) };
			locEntry.rooms.length = 0;

			for (const room of rooms) {
				const appliances = await this._getAppliances(loc.id, room.id);
				const roomEntry = { id: room.id, appliances: appliances.slice(0) };
				roomEntry.appliances.length = 0;
				...
				for (const app of appliances) {
					...
					try {
						...
						this.log.debug(`[client] Failed to get notifications for ${app.appliance_id}: ${err.message}`);
					}
	...
	async _getLocations() {
		const resp = await this.request({ method: 'GET', url: `${BASE}/locations` });
		return resp.data || [];
	}
